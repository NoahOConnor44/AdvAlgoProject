...
def MCPM(T):
    ...
    # create subgraph
    print("Subgraph: ")
    T_subgraph = nx.Graph(T.subgraph(odd_deg))
...
def euler_tour(MT, G):
    et = nx.algorithms.eulerian_circuit(MT)
    e_tour = []
    for x in et:
        e_tour.append(x)
    print("Euler tour:", et)
    ...
    for edge in e_tour:
        euler_graph.add_edge(edge[0], edge[1], G.get_edge_data(edge[0], edge[1])["weight"])
...
def TS_Tour(eulertour, G):
    ...
    for x in range(len(eulertour)-1):
        TST.add_edge(eulertour[x],euler_tour[x+1], G.get_edge_data(eulertour[x],euler_tour[x+1])["weight"])
    ...
    for x in range(1, len(eulertour)):
        c += int(G.get_edge_data(eulertour[x-1],euler_tour[x])["weight"])
    print("Cost: ", c)
    
def main():
    in_txt = r"DATA\32TSP\input3.txt"
    G = read_input(in_txt)
    print("Input Graph:")
    print_graph(G)
